...
module.exports = async (deployer) => {
    ...
    if (deployer.network === 'mainnet') {
        const key1 = await crossChainBridge.getBridgeKey(aEthContractAddress['mainnet'], aEthContractAddress['smartchain'], web3.utils.numberToHex(chainNetwork['mainnet']), web3.utils.numberToHex(chainNetwork['smartchain']));
        await crossChainBridge.changeBridgeStatus(key1, ENABLED_BRIDGE);
        const key2 = await crossChainBridge.getBridgeKey(aEthContractAddress['mainnet'], aEthContractAddress['smartchain'], web3.utils.numberToHex(chainNetwork['mainnet']), web3.utils.numberToHex('57'));
        await crossChainBridge.changeBridgeStatus(key2, DISABLED_BRIDGE);
    } else {
        return;
    }
